 ...
 * @property {string[]} images - 상품 이미지 배열
 */

/**
 * 별점 아이콘 HTML을 생성하는 헬퍼 함수
 * @param {number} rating - 평점 (0-5)
 * @returns {string} 별점 아이콘 HTML
 */
const generateStarRating = (rating) => {
  ...
  const fullStarIcon = `<svg class="w-4 h-4 text-yellow-400" fill="currentColor" viewBox="0 0 20 20"><path d="${starPath}"></path></svg>`;
  const emptyStarIcon = `<svg class="w-4 h-4 text-gray-300" fill="currentColor" viewBox="0 0 20 20"><path d="${starPath}"></path></svg>`;

  return fullStarIcon.repeat(fullStars) + emptyStarIcon.repeat(emptyStars);
};

/**
 * @param {Object} props
 * @param {boolean} props.loading - 로딩 상태
 * @param {Product} props.product - 상품 상세 정보
