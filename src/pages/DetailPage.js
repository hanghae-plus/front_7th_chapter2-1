...
export default function DetailPage({ loading, error, product, relatedProducts, cartCount = 0 }) {
  if (loading) {
    return renderLoading(cartCount);
  }

  if (error) {
    return renderError(error, cartCount);
  }

  if (!product) {
    return renderNotFound(cartCount);
  }

  return renderDetail(product, relatedProducts, cartCount);
}

// 로딩 중 UI
function renderLoading(cartCount) {
  const content = /*html*/ `
...
// 에러 UI
function renderError(error, cartCount) {
  const content = /*html*/ `
    <div class="py-20 bg-gray-50 flex items-center justify-center">
      <div class="text-center">
        ...
        <h3 class="text-lg font-medium text-gray-900 mb-2">상품 정보를 불러올 수 없습니다</h3>
        <p class="text-gray-600 mb-4">${error.message || "오류가 발생했습니다"}</p>
        <a href="/" data-link="" class="text-blue-600 hover:text-blue-800">홈으로 돌아가기</a>
      </div>
    </div>
  `;
...
// 상품을 찾을 수 없음
function renderNotFound(cartCount) {
  const content = /*html*/ `
    <div class="py-20 bg-gray-50 flex items-center justify-center">
      <div class="text-center">
        ...
        <p class="text-gray-600 mb-4">해당 상품이 존재하지 않거나 삭제되었습니다</p>
        <a href="/" data-link="" class="text-blue-600 hover:text-blue-800">홈으로 돌아가기</a>
      </div>
    </div>
  `;
...
// 상세페이지 UI
function renderDetail(product, relatedProducts = [], cartCount) {
  const stars = renderStars(product.rating);
  const breadcrumb = renderBreadcrumb(product);
...
function renderStars(rating) {
  ...
  const emptyStarIcon = /*html*/ `
    <svg class="w-4 h-4 text-gray-300" fill="currentColor" viewBox="0 0 20 20">
      <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"></path>
    </svg>
  `;

  return starIcon.repeat(fullStars) + emptyStarIcon.repeat(emptyStars);
}
