...
export const ProductDetail = () => {
  ...
  return /*html*/ `
  ${isLoading ? ProductDetail.Loading() : ProductDetail.Container()}
  `;
};
...
ProductDetail.Breadcrumb = () => {
  const { category1, category2 } = store.getState("selectedProduct");
...
ProductDetail.Item = () => {
  const { title, image, lprice, brand, description, rating, reviewCount, stock } = store.getState("selectedProduct");
...
ProductDetail.Cart = () => {
  const { productId } = store.getState("selectedProduct");
  const { quantity } = store.getState("cart");
  return /*html*/ `
...
ProductDetail.RelatedItem = (detail) => {
  const { productId, title, image, lprice } = detail;
