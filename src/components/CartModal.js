export default function CartModal({ items = [], selectedIds = [] }) {
  if (items.length === 0) {
    return renderEmpty();
  }

  const hasSelected = selectedIds.length > 0;
  return hasSelected ? renderWithSelection(items, selectedIds) : renderWithoutSelection(items);
}
...
// 선택 없음
function renderWithoutSelection(items) {
        ...
        <div class="flex flex-col max-h-[calc(90vh-120px)]">
          ...
          <div class="flex-1 overflow-y-auto">
            <div class="p-4 space-y-4">
              ${items.map((item) => renderCartItem(item, false)).join("")}
            </div>
          </div>
        </div>
        <!-- 하단 액션 -->
...
// 선택 있음
function renderWithSelection(items, selectedIds) {
  const selectedItems = items.filter((item) => selectedIds.includes(item.productId));
  const selectedPrice = selectedItems.reduce((sum, item) => sum + parseInt(item.price) * item.quantity, 0);
  const totalPrice = items.reduce((sum, item) => sum + parseInt(item.price) * item.quantity, 0);
    ...
    <div class="flex min-h-full items-end justify-center p-0 sm:items-center sm:p-4">
      <div class="cart-modal relative bg-white rounded-t-lg sm:rounded-lg shadow-xl w-full max-w-md sm:max-w-lg max-h-[90vh] overflow-hidden">
        ...
        <div class="flex flex-col max-h-[calc(90vh-120px)]">
          ...
          <div class="p-4 border-b border-gray-200 bg-gray-50">
            <label class="flex items-center text-sm text-gray-700">
              <input type="checkbox" id="cart-modal-select-all-checkbox" class="w-4 h-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500 mr-2" ${selectedIds.length === items.length ? "checked" : ""}>
              전체선택 (${items.length}개)
            </label>
          </div>
          <!-- 아이템 목록 -->
          <div class="flex-1 overflow-y-auto">
            <div class="p-4 space-y-4">
              ${items.map((item) => renderCartItem(item, selectedIds.includes(item.productId))).join("")}
            </div>
          </div>
        </div>
        <!-- 하단 액션 -->
        <div class="sticky bottom-0 bg-white border-t border-gray-200 p-4">
          ...
          <div class="flex justify-between items-center mb-3 text-sm">
            <span class="text-gray-600">선택한 상품 (${selectedIds.length}개)</span>
...
// 장바구니 아이템
function renderCartItem(item, isSelected) {
    ...
    <div class="flex items-center py-3 border-b border-gray-100 cart-item" data-product-id="${item.productId}">
      ...
      <label class="flex items-center mr-3">
        <input type="checkbox" class="cart-item-checkbox w-4 h-4 text-blue-600 border-gray-300 rounded 
          focus:ring-blue-500" data-product-id="${item.productId}" ${isSelected ? "checked" : ""}>
      </label>
      <!-- 상품 이미지 -->
      <div class="w-16 h-16 bg-gray-100 rounded-lg overflow-hidden mr-3 flex-shrink-0">
        <img src="${item.image}" alt="${item.name}" class="w-full h-full object-cover cursor-pointer cart-item-image" data-product-id="${item.productId}">
      </div>
      <!-- 상품 정보 -->
      <div class="flex-1 min-w-0">
        ...
        <div class="flex items-center mt-2">
          ...
          <input type="number" value="${item.quantity}" min="1" class="cart-quantity-input w-12 h-7 text-center text-sm border-t border-b 
