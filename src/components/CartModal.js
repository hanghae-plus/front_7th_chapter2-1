...
const CartItem = (item) => {
  const { productId, image, title, price, quantity, isSelected = false } = item;
  return `
  <div class="flex items-center py-3 border-b border-gray-100 cart-item" data-product-id="${productId}">
    ...
    focus:ring-blue-500" data-product-id="${productId}"${isSelected ? " checked" : ""}>
    </label>
    <!-- 상품 이미지 -->
...
export const CartModal = () => {
  const { cartList, isCartModalOpen } = store.state;

  if (!isCartModalOpen) {
    return "";
  }

  const selectedItems = cartList.filter((cart) => cart.isSelected);
  const selectedCount = selectedItems.length;
  const selectedTotal = selectedItems.reduce((sum, item) => sum + item.price * item.quantity, 0);
  const isAllSelected = cartList.length > 0 && cartList.every((cart) => cart.isSelected);
  return `
  <div class="fixed inset-0 z-50 overflow-y-auto" id="cart-modal-container">
    <!-- 배경 오버레이 -->
    <div class="fixed inset-0 bg-black bg-opacity-50 transition-opacity" id="cart-modal-overlay"></div>
    <!-- 모달 컨텐츠 -->
    <div class="flex min-h-full items-end justify-center p-0 sm:items-center sm:p-4">
      ...
      <!-- 컨텐츠 -->
      <div class="flex flex-col max-h-[calc(90vh-242px)]">
      ${
        cartList.length > 0
          ? `
        <!-- 전체 선택 섹션 -->
        <div class="p-4 border-b border-gray-200 bg-gray-50">
          <label class="flex items-center text-sm text-gray-700">
            ...
            전체선택 (${cartList.length}개)
          </label>
        </div>
        <!-- 아이템 목록 -->
        <div class="flex-1 overflow-y-auto">
          <div class="p-4 space-y-4">
          ${cartList.map((cart) => CartItem(cart)).join("")}
          </div>
        </div>
        `
          : `
      ...
      ${
        ...
        <div class="sticky bottom-0 bg-white border-t border-gray-200 p-4">
          ...
          <div class="flex justify-between items-center mb-4">
            ...
            <span class="text-xl font-bold text-blue-600">${cartList
