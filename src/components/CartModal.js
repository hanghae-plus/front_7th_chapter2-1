...
const ModalLayout = ({ children, cartList }) => {
  return /* HTML */ `
        ...
        >
          ...
          <div class="sticky top-0 bg-white border-b border-gray-200 p-4 flex items-center justify-between">
            <h2 class="text-lg font-bold text-gray-900 flex items-center">
              ...
              장바구니 ${cartList.length ? `(${cartList.length}개)` : ""}
            </h2>
...
const cartItem = ({ id, image, price, title, quantity, selected }) => {
  return /* HTML */ `
    <div class="flex items-center py-3 border-b border-gray-100 cart-item" data-product-id=${id}>
      ...
      <div class="text-right ml-3">
        <p class="text-sm font-medium text-gray-900">${(Number(price) * quantity).toLocaleString()}원</p>
...
const modalBody = ({ cartList, selectedAll }) => {
  const totalCount = cartList.length;
  const totalPrice = cartList.reduce((acc, cur) => acc + cur.price * cur.quantity, 0);
  const checkedList = cartList.filter((item) => item.selected);
  const checkedPrice = checkedList.reduce((acc, cur) => acc + cur.price * cur.quantity, 0);
  ...
  return /* HTML */ `
    ...
    <div class="flex flex-col max-h-[calc(90vh-120px)]">
      ...
      <div class="flex-1 overflow-y-auto">
        <div class="p-4 space-y-4">${cartList.map((item) => cartItem({ ...item })).join("")}</div>
      </div>
    </div>
    <!-- 하단 액션 -->
      ...
      ${checkedList.length >= 1
        ? /* HTML */ `
            <div class="flex justify-between items-center mb-3 text-sm">
              <span class="text-gray-600">선택한 상품 (${checkedList.length}개)</span>
      ...
      <div class="space-y-2">
        ${checkedList.length >= 1
          ? /* HTML */ `
              ...
              >
                선택한 상품 삭제 (${checkedList.length}개)
              </button>
            `
          : ``}
...
export const CartModal = () => {
  const { items: cartList, selectedAll } = cartState.getCart();

  return ModalLayout({
    children: cartList.length >= 1 ? modalBody({ cartList, selectedAll }) : emptyCartUI(),
    cartList,
    selectedAll,
  });
};
...
export const renderCartModal = () => {
  ...
  if (existingModal) {
    const newModalHTML = CartModal();
    const tempDiv = document.createElement("div");
    tempDiv.innerHTML = newModalHTML;

    existingModal.replaceWith(tempDiv.firstElementChild);
  }
};
