export default function CartItem(item) {
  const { productId, title, image, lprice, quantity, isChecked } = item;
  const itemTotalPrice = lprice * quantity;
  ...
  return `
    <div class="flex items-center py-3 border-b border-gray-100 cart-item" data-product-id="${productId}">
      ...
      <label class="flex items-center mr-3">
        ...
        focus:ring-blue-500" data-product-id="${productId}" ${isChecked ? "checked" : ""}>
      </label>
      <!-- 상품 이미지 -->
