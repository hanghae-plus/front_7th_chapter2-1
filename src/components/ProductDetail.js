...
export const ProductDetail = () => {
  ...
  <div class="bg-white rounded-lg shadow-sm mb-6">
    ...
    <div class="p-4">
      ...
      <!-- 상품 정보 -->
      <div>
        ${brand ? `<p class="text-sm text-gray-600 mb-1">${brand}</p>` : ""}
        <h1 class="text-xl font-bold text-gray-900 mb-3">${title}</h1>
        <!-- 평점 및 리뷰 -->
        <div class="flex items-center mb-3">
          <div class="flex items-center">
            ${filledReview.repeat(rating)}
            ${emptyReview.repeat(5 - rating)}
          </div>
          <span class="ml-2 text-sm text-gray-600">${rating}.0 (${Number(reviewCount).toLocaleString()}개 리뷰)</span>
        </div>
        <!-- 가격 -->
        <div class="mb-4">
