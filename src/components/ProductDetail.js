...
const StarRating = ({ rating = 4 }) => {
  ...
  for (let i = 1; i <= 5; i++) {
    ...
    stars.push(`
      <svg class="w-4 h-4 ${isFilled ? "text-yellow-400" : "text-gray-300"}" fill="currentColor" viewBox="0 0 20 20">
...
const RelatedProductCard = ({ id, name, price, imageUrl }) => `
  <div class="bg-gray-50 rounded-lg p-3 related-product-card cursor-pointer" data-product-id="${id}">
    <div class="aspect-square bg-white rounded-md overflow-hidden mb-2">
      <img src="${imageUrl}" alt="${name}" class="w-full h-full object-cover" loading="lazy">
    </div>
    <h3 class="text-sm font-medium text-gray-900 mb-1 line-clamp-2">${name}</h3>
    ...
    <div class="bg-white rounded-lg shadow-sm mb-6">
      ...
      <div class="p-4">
        <div class="aspect-square bg-gray-100 rounded-lg overflow-hidden mb-4">
          <img src="${imageUrl}" alt="${name}" class="w-full h-full object-cover product-detail-image">
        </div>
        <!-- 상품 정보 -->
        <div>
      ...
      <div class="border-t border-gray-200 p-4">
        ...
        <!-- 액션 버튼 -->
        <button id="add-to-cart-btn" data-product-id="${id}" class="w-full bg-blue-600 text-white py-3 px-4 rounded-md 
